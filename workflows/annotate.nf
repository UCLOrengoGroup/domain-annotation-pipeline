...
workflow {
    ...
    def chain_filter = run_filter_domains(all_chainsaw_results)
    // filter chainsaw results for inclusion in consensus
    ...
    def chop = chop_pdb(consensus.filtered, pdb_ch.collect())
    //Use filtered_consensus.tsv to chop the pdb files accordingly
    def md5 = create_md5(chop.chop_files.flatten())
    // creates an domain-level md5 has from the seq in the chopped pdb files
    def combined_md5 = md5.collectFile(name: "all_md5.tsv", sort: true)
    def stride = run_stride(chop.chop_files)
    // Run STRIDE on each chopped pdb domain file
    def globularity = run_measure_globularity(chop.chop_dir)
    // Run globularity in the chopped pdb files
    def summaries = summarise_stride(stride.flatten())
