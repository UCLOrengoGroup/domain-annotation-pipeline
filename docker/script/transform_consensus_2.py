...
def transform_consensus(input_file, output_file, stride_files):
    ...
    for idx, row in df.iterrows():
        ...
        for level in ['high', 'med']:
            ...
            for i, domain in enumerate(domains, 1):
                ...
                # Get the expected filename and lookup path
                stride_filename = f"{pdb_id}_{level}_{i}.stride.summary"
    ...
    column_names = ['ted_id', 'md5_domain', 'consensus_level', 'chopping', 'nres_domain', 'num_segments'] + \
                   [f'{k}' for k in stride_keys]
